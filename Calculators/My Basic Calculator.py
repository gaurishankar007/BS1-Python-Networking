...
def click_number(number):  # A function to update the Expression in the text entry box
    ...
    New_Expression.set(Expression)  # Updating the Expression by set method
...
New_Expression = StringVar()  # StringVar() is the variable class we create an instance of this class
# That means the str value of this variable can be changeable

Screen = Entry(Frm0, textvariable=New_Expression, font=('Chiller', 38, 'bold'), bg='purple', relief=GROOVE, border=0)
Screen.pack(expand=True, fill=BOTH)

New_Expression.set("Start Calculating")
...
Zero = Button(Frm5, text="0", height=3, width=37, activebackground='green', font=('chiller', 15, 'bold'), relief=GROOVE, border=0, command=lambda: click_number(0))
Zero.pack(side=LEFT)
